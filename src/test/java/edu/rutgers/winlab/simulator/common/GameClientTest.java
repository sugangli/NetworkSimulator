...
public class GameClientTest {
    ...
    public void test1() {
        ...
        final GameClient c1 = new edu.rutgers.winlab.simulator.traditionalgaming.GameClient("C1", new FIFOQueue<>("C1_IN", Integer.MAX_VALUE), new Consumer<UserEvent>() {
            @Override
            public void accept(UserEvent evt) {
                LinkedList<Long> l = packetDurations.get(evt.getId());
                l.addLast(EventQueue.now());
            }
        });
        for (int i = 0; i < 100; i++) {
            EventQueue.addEvent(EventQueue.now() + i * EventQueue.MILLI_SECOND * 10, new Action() {
                @Override
        ...
        packetDurations.forEach(new BiConsumer<Integer, LinkedList<Long>>() {
            @Override
            public void accept(Integer k, LinkedList<Long> v) {
